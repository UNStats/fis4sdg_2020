...
catalog = availability.available_geo_by_series(folder)
...
for entry in catalog:
    ...
    for geo in entry['geoAreaCodes']:
        ...
        for k in dataset[0].keys():
            ...
            if utils.camel_case(k) in concepts:
                ...
                codelist_k = copy.deepcopy(utils.select_dict(codes, {'concept': utils.camel_case(k)})[0]['codes'])
                ...
                new_dataset = utils.merge_dict_lists(new_dataset, codelist_k , [k], ['x'], how='left')
                
                new_dataset = utils.subdict_list(new_dataset, [k, 'x'], exclude=True)
    ...
    series_data = utils.merge_dict_lists(series_data, geo_tree, ['geoAreaCode'], ['geoAreaCode'], how='left')
        
    file_out = 'data/interim/' + release + '/Series_' +  series + '.txt'
